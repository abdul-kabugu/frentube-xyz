    ...
    const CREATOR_USER_ID = data?.postById?.createdByAccount?.id
      //  CHECK  IF  CURRENT  USER  IS  SUBSCRIBER 
       ...
       /* const checkIsFollower = async () =>  {
        ...
        const isFollower = await substrateApi?.isAccountFollower(currentUserInfo?.address, CREATOR_USER_ID)
             ...
             <Box display="flex" w="100%" gap={6} alignItems="center" justifyContent="space-between" >
               <HStack gap={4}>
                <Avatar size="md" src={`${IPFS_GATEWAY}${data?.postById?.createdByAccount.profileSpace?.image}`}  />
                  <Box>
                   <Link href={`/channels/${data?.postById?.createdByAccount?.id}`}> <Text fontSize="lg" fontWeight="semibold">{ data?.postById?.createdByAccount.profileSpace?.name ||  data?.postById?.createdByAccount && shortenTxt(data?.postById?.createdByAccount?.id, 10)}</Text> </Link>
