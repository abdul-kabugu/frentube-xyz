  ...
  return (
    <Box display="flex" gap={4} my={6} >
       <Avatar size="md"    />
         <Box>
             <HStack gap={2}>
                <Link href={`/channels/${comment?.createdByAccount?.id}`}> <Text fontWeight="semibold">{comment?.createdByAccount?.id && shortenTxt(comment?.createdByAccount?.id, 10)}</Text> </Link>
